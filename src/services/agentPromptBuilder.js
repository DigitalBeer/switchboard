...
exports.buildKanbanBatchPrompt = buildKanbanBatchPrompt;
exports.columnToPromptRole = columnToPromptRole;
function buildReviewerExecutionIntro(planCount) {
    if (planCount <= 1) {
...
function buildReviewerExecutionModeLine(expectation) {
    return `Mode:
...
function withCoderAccuracyInstruction(basePayload, enabled) {
    if (!enabled) {
        return basePayload;
    }
 ...
 */
function buildKanbanBatchPrompt(role, plans, options) {
...
- do NOT start \`/challenge\` or any auxiliary workflow for this step.`;
    ...
    const chatCritiqueDirective = `When you output the adversarial critique (Grumpy and Balanced sections), include them verbatim in your chat response as formatted markdown — do not only write them to the plan file. The user must be able to read the critique directly in chat without opening the plan.`;
 ...
 */
function columnToPromptRole(column) {
